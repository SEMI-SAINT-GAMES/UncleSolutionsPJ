...
async def read_articles(mongodb=Depends(get_mongodb),pagination: Pagination = Depends(), search: str | None = None, tags: str | None = None):
    pipeline = []
    ...
    total = await mongodb["articles"].count_documents(pipeline[0]["$match"] if pipeline else {})
    pipeline += [
        {
            "$lookup": {
                "from": "users",
...
@article_router.get("/by_user_id/{user_id}", response_model=PaginationResponseModel[ArticleOut])
async def read_articles_by_username(user_id: str, mongodb=Depends(get_mongodb), pagination: Pagination = Depends()):
    pipeline = [
        {
            "$lookup": {
                "from": "users",
