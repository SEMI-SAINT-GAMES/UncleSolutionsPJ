...
from app.models import VerifyModel, VerifyRequest, ForgotPasswordRequest, ResetPasswordRequest, UsernameRequest
...
def _make_tokens(username: str, user_id: str | None = None) -> dict[str, str]:
    access = create_jwt_token({"sub": username, "_id": str(user_id) if user_id else None})
    refresh = create_jwt_token({"sub": username, "_id": str(user_id) if user_id else None}, expires=1440)
...
async def verify_user(verify: VerifyRequest, mongodb=Depends(get_mongodb)):
    if not verify.username or not verify.code:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Username and code are required")

    record = await mongodb["verify_codes"].find_one_and_delete({"username": verify.username})
    if not record:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No verification code found or it expired")

    if not check_verification_data(record, verify.code):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid verification code")

    await mongodb["users"].update_one({"username": verify.username}, {"$set": {"is_active": True}})
...
async def forgot_password_request(req: ForgotPasswordRequest, mongodb=Depends(get_mongodb)):
    user = await mongodb["users"].find_one({"email": req.email})
    if not user:
        return JSONResponse(status_code=status.HTTP_200_OK, content={"message": "If an account with this email exists, a reset code has been sent."})
...
async def forgot_password_verify(payload: ResetPasswordRequest, mongodb=Depends(get_mongodb)):
    ...
    await mongodb["users"].update_one({"username": verify.username}, {"$set": {"password": hashed_password}})
